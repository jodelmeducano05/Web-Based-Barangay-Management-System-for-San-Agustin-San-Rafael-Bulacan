...
const COLORS = {
  primary: "#213359", // Navy Blue - #213359
  secondary: "#4ADE80", // Green - #4ADE80
};
const INITIAL_USERS = [
...
export default function App() {
  ...
  const [documents, setDocuments] = useState([
    ...
    { id: "DOC-003", resident: "Pedro Reyes", type: "Certificate of Indigency", date: "September 12, 2026", status: "Verified" },
  ]);
  ...
  const [blotters, setBlotters] = useState([
    ...
    { id: "BR-0010", complainant: "Pedro Reyes", incident: "Property Concern", date: "September 12, 2026", status: "Pending" },
  ]);
  const showToast = (msg) => {
    setToast(msg);
    ...
    <div style={styles.loginPage}>
      <div style={styles.loginCard}>
        ...
        <div style={{background:'#f1f5f9', padding:'10px', borderRadius:'8px', marginBottom:'15px', fontSize:'11px', textAlign:'left'}}>
          <b>Demo Accounts (password: res123,cap123,sec123):</b><br/> resident@gmail.com - Resident<br/> captain@gmail.com - Captain<br/> secretary@gmail.com - Secretary
        </div>
...
function DashboardModules({ onSelect, logout, user }) {
  const modules = [
    { name: "Resident Dashboard", role: "Resident", icon: "👥", color: COLORS.primary, description: "Access resident services, document requests, announcements, and blotter reports in SAN AGUSTINE." },
    { name: "Captain Dashboard", role: "Captain", icon: "📊", color: COLORS.secondary, description: "Manage barangay operations, residents, documents, announcements, and reports for SAN AGUSTINE." },
    { name: "Secretary Dashboard", role: "Secretary", icon: "📝", color: COLORS.primary, description: "Manage records, document requests, announcements, and barangay reports of SAN AGUSTINE." },
  ];
  return (
    <div style={styles.modulePage}>
      <header style={styles.moduleTopbar}>
        ...
        <div style={styles.userArea}><div style={styles.userInfo}><strong>{user.email}</strong><span style={{background: COLORS.secondary, color: COLORS.primary, padding:'2px 8px', borderRadius:'20px', fontSize:'10px', marginTop:'3px', fontWeight:'bold'}}>{user.role}</span></div><button style={styles.logoutButton} onClick={logout}>Logout</button></div>
      </header>
          ...
          {modules.map((m) => {
            ...
            return (
              <div key={m.name} style={{...styles.moduleCard, opacity: isAllowed ? 1 : 0.6, border: isAllowed ? `2px solid ${m.color}` : '1px solid #e2e8f0', cursor: isAllowed ? 'pointer' : 'not-allowed'}} onClick={() => onSelect(m.name)}>
                ...
                {isAllowed && <div style={{...styles.lockedBadge, background: COLORS.secondary, color: COLORS.primary}}>✅ ALLOWED FOR YOU</div>}
    ...
    <div style={styles.dashboardLayout}>
      <aside style={styles.sidebar}>
        ...
        <nav style={styles.sidebarNav}><p style={styles.menuLabel}>MAIN MENU</p>
          {menuItems.map((item) => (
            <button key={item.name} onClick={() => { setActivePage(item.name); showToast(`Opened ${item.name}`); }} style={{ ...styles.sidebarMenu, ...(activePage === item.name ? styles.sidebarMenuActive : {}) }}><span>{item.icon}</span><span>{item.name}</span></button>
          ))}
        </nav>
...
function DashboardHome({ role, selectedModule, setActivePage, showToast }) {
  const stats = [
    { title: "Document Requests", value: "24", icon: "📄", color: COLORS.primary },
    { title: "Announcements", value: "8", icon: "📢", color: COLORS.secondary },
    { title: "Blotter Reports", value: "12", icon: "📋", color: COLORS.primary },
    { title: "Pending Requests", value: "6", icon: "⏳", color: COLORS.secondary },
  ];
  return (
    <div>
      <div style={{...styles.dashboardWelcome, background:`linear-gradient(135deg, ${COLORS.primary}, ${COLORS.secondary})`}}><div><h2>Welcome, {role}!</h2><p>You are using <strong>{selectedModule}</strong> - {BARANGAY}</p></div><div style={styles.dateBox}><span>📅</span><span>{new Date().toLocaleDateString()}</span></div></div>
      ...
      <div style={styles.sectionGrid}>
        ...
        <div style={styles.contentCard}><h2>Recent Activity</h2>
          <div style={styles.activityList}>
            <div style={styles.activityItem}><span style={{...styles.dot, background: COLORS.primary}}></span><div><strong>New document request</strong><p style={{margin:'4px 0', fontSize:'12px', color:'#64748b'}}>Barangay Clearance - SAN AGUSTINE</p><small>Today, 9:30 AM</small></div></div>
            <div style={styles.activityItem}><span style={{...styles.dot, background: COLORS.secondary}}></span><div><strong>New announcement posted</strong><p style={{margin:'4px 0', fontSize:'12px', color:'#64748b'}}>Barangay meeting - SAN AGUSTINE</p><small>Yesterday, 3:00 PM</small></div></div>
            <div style={styles.activityItem}><span style={{...styles.dot, background: COLORS.primary}}></span><div><strong>Blotter report updated</strong><p style={{margin:'4px 0', fontSize:'12px', color:'#64748b'}}>Report #BR-0012</p><small>Yesterday, 10:15 AM</small></div></div>
          </div>
        </div>
      </div>
    </div>
...
// ===== BINAGO DITO - CAPTAIN APPROVE/REJECT, SECRETARY VERIFIED =====
function DocumentsPage({ role, documents, setDocuments, showToast }) {
  ...
  const handleStatusChange = (docId, newStatus) => {
    if (newStatus === "Verified" && role !== "Secretary") {
      return showToast("⛔ Si Secretary lang pwede mag Verified!");
    }
    if ((newStatus === "Approved" || newStatus === "Rejected") && role !== "Captain") {
      return showToast("⛔ Si Captain lang pwede mag Approve/Reject!");
    }
    ...
    showToast(`Status: ${newStatus} - ${BARANGAY}`);
  };
  ...
  return (
    <div>
      <div style={styles.pageHeader}><div><h2>Document Requests - {BARANGAY}</h2><p>Manage barangay document requests.</p></div>
        {role === "Resident" && <button style={styles.primaryButton} onClick={() => setShowForm(!showForm)}>{showForm ? "Close Form" : "+ New Request"}</button>}
      </div>
      ...
      {showDetail && (
        <div style={styles.modalOverlay} onClick={() => setShowDetail(null)}><div style={styles.modal} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:'flex', gap:'8px', marginTop:'15px', flexWrap:'wrap'}}>
            ...
            {/* SECRETARY ONLY */}
            {role === "Secretary" && showDetail.status === "Pending" && (
              <button style={{...styles.primaryButton, background: '#3b82f6', color:'white'}} onClick={() => handleStatusChange(showDetail.id, "Verified")}>✓ Mark as Verified</button>
            )}
            {role === "Secretary" && showDetail.status !== "Pending" && (
              <div style={{fontSize:'12px', color:'#64748b', padding:'8px', background:'#f1f5f9', borderRadius:'6px', width:'100%'}}>Info: Na-process mo na ito ({showDetail.status}). Si Captain na ang mag Approve/Reject.</div>
            )}

            {/* CAPTAIN ONLY */}
            {role === "Captain" && showDetail.status === "Verified" && (
              <>
                <button style={{...styles.primaryButton, background: '#22c55e', color:'white'}} onClick={() => handleStatusChange(showDetail.id, "Approved")}>✔ Approve Document</button>
                <button style={{...styles.primaryButton, background: '#ef4444', color:'white'}} onClick={() => handleStatusChange(showDetail.id, "Rejected")}>✖ Reject Document</button>
              </>
            )}
            {role === "Captain" && showDetail.status === "Pending" && (
              <div style={{fontSize:'12px', color:'#ef4444', padding:'8px', background:'#fef2f2', borderRadius:'6px', width:'100%'}}>⛔ Hindi mo pa pwede i-approve. Hintayin ma-verify ni Secretary.</div>
            )}
            {role === "Captain" && (showDetail.status === "Approved" || showDetail.status === "Rejected") && (
              <div style={{fontSize:'12px', color:'#16a34a', padding:'8px', background:'#f0fdf4', borderRadius:'6px', width:'100%'}}>Tapos na: {showDetail.status}</div>
            )}

            {role === "Resident" && (
              <div style={{fontSize:'12px', color:'#64748b', padding:'8px', background:'#f8fafc', borderRadius:'6px', width:'100%'}}>Status: {showDetail.status} - Antayin ang verification ni Secretary at approval ni Captain.</div>
            )}
    ...
    <div>
      ...
      <div style={styles.announcementGrid}>{announcements.map(a => (
        <div style={styles.announcementCard} key={a.id}>
          ...
          <div><h3>{a.title}</h3><p style={{color: COLORS.secondary, fontSize:'12px'}}> {a.date} • {BARANGAY}</p><p style={{fontSize:'13px', color:'#475569'}}>{a.description}</p>
...
const styles = {
  ...
  loginTitle: { color:COLORS.primary, fontSize:"22px", margin:"0 0 8px" },
  loginSubtitle: { color:COLORS.secondary, fontSize:"13px", marginBottom:"15px", fontWeight:'bold' },
  ...
  googleIcon: { fontWeight:"bold", fontSize:"18px", color:COLORS.secondary },
  ...
  smallLogo: { width:"45px", height:"45px", borderRadius:"12px", objectFit:"cover", border:`2px solid ${COLORS.secondary}` },
  brandTitle: { margin:0, fontSize:"18px", color:COLORS.primary },
  brandSubtitle: { margin:"4px 0 0", fontSize:"11px", color:COLORS.secondary, fontWeight:'bold' },
  ...
  sidebarSubtitle: { margin:"3px 0 0", color:COLORS.secondary, fontSize:"10px", fontWeight:"bold" },
  ...
  menuLabel: { color:COLORS.secondary, fontSize:"11px", fontWeight:"bold", padding:"0 12px", marginBottom:"5px", letterSpacing:"1px" },
  ...
  pageTitle: { margin:0, color:COLORS.primary, fontSize:"22px" },
  dashboardUser: { display:"flex", alignItems:"center", gap:"10px" },
  avatar: { width:"42px", height:"42px", borderRadius:"50%", background:COLORS.secondary, color:COLORS.primary, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:"bold", fontSize:"18px", border:`2px solid ${COLORS.primary}` },
  pageContent: { padding:"30px 35px" },
  dashboardWelcome: { background:`linear-gradient(135deg, ${COLORS.primary}, ${COLORS.secondary})`, color:"#ffffff", borderRadius:"16px", padding:"25px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:"20px", flexWrap:"wrap", marginBottom:"25px" },
