...
export default function App() {
  ...
  const handleLogin = (email, password) => {
    ...
    setUser(found); setStage("modules");
  };
  const handleGoogleLogin = () => { setUser({ email: "Google User", role: "Resident" }); setStage("modules"); };
...
function Login({ onLogin, onGoogleLogin, users, setUsers, showToast }) {
  ...
  return (
    <div style={styles.loginPage}><div style={styles.loginCard}>
      ...
      <div style={{background:'#f1f5f9', padding:'10px', borderRadius:'8px', marginBottom:'15px', fontSize:'11px', textAlign:'left'}}><b>Demo:</b> resident / res123 | captain / cap123 | secretary / sec123<br/><span style={{color:'#16a34a', fontWeight:'bold'}}>Code: CAPTAIN2026 / SECRETARY2026</span></div>
    ...
    <div style={styles.dashboardLayout}>
      <aside style={styles.sidebar}>
        ...
        <div style={styles.sidebarBottom}>
          {/* Change Module - TINANGGAL NA LAHAT */}
...
function DocumentsPage({ role, documents, setDocuments, showToast }) {
  ...
  const handleStatusChange = (docId, newStatus) => {
    if (newStatus === "Verified" && role !== "Secretary") return showToast("⛔ Secretary lang!");
    if ((newStatus === "Approved" || newStatus === "Rejected") && role !== "Captain") return showToast("⛔ Captain lang!");
    setDocuments(documents.map(d=> d.id===docId ? {...d, status:newStatus} : d)); if(showDetail) setShowDetail({...showDetail, status:newStatus});
  };
...
function BlotterReportsPage({ role, blotters, setBlotters, showToast }) {
  ...
  const handleBlotterStatus = (id, newStatus) => {
    if ((newStatus === "Under Review") && role !== "Secretary") return showToast("⛔ Secretary lang!");
    if ((newStatus === "Resolved" || newStatus === "Dismissed") && role !== "Captain") return showToast("⛔ Captain lang!");
