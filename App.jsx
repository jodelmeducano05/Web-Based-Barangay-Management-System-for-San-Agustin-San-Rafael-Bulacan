...
onMouseEnter={(e) => { if(activePage !== item.name) e.currentTarget.style.background = 'rgba(74,222,128,0.15)'}}
...
{["Pending","Processing","Approved"].map(s => <button key={s} style={{...styles.viewButton, background: showDetail.status===s ? COLORS.secondary : '#dcfce7', color: showDetail.status===s ? COLORS.primary : COLORS.primary}} onClick={() => { setDocuments(documents.map(d=> d.id===showDetail.id ? {...d, status:s} : d)); setShowDetail({...showDetail, status:s}); showToast(`Status changed to ${s}`); }}>{s}</button>)}
